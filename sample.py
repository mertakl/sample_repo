class VectorDBManager:
    ...
    # ... (other methods like update_for_language, _initialize_vector_db, etc., remain unchanged) ...

    def _convert_doc_to_docx_locally(self, doc_path: Path, output_dir: Path) -> Path | None:
        """Converts a .doc file to .docx using LibreOffice and returns the new path."""
        try:
            # Use subprocess to call LibreOffice's command-line tool
            subprocess.run(
                ["lowriter", "--headless", "--convert-to", "docx", str(doc_path), "--outdir", str(output_dir)],
                check=True,
                capture_output=True, # Capture stdout/stderr
            )
            # The output file will have the same basename with a .docx extension
    ...
    async def _parse_document_content(
        ...
        # Use a temporary directory to handle original, converted, and output files
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_dir_path = Path(temp_dir)
            
            # This is the path the parser will read from
            path_to_parse: Path

            if original_extension == "doc":
                # --- CONVERSION LOGIC FOR .DOC FILES ---
                # 1. Write the original .doc content to a temporary file
                temp_doc_path = temp_dir_path / file_name
                temp_doc_path.write_bytes(content)

                # 2. Convert it to .docx inside the same temporary directory
                ...
                # Write original content to a temp file with the correct suffix
                temp_file_path = temp_dir_path / file_name
                temp_file_path.write_bytes(content)
                path_to_parse = temp_file_path

            # 3. Parse the final document (either original or the converted .docx)
