"""Refactored SharePoint Client with improved structure and best practices."""
...
class SharePointAuthenticator:
    ...
    def __init__(self, config: 'SharePointConfig', azure_creds: 'AzureCredentials'):
        self.config = config
...
class SharePointAPIClient:
    ...
    def send_request(self, endpoint: str) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                url, 
                headers=headers, 
                proxies=self.authenticator._get_proxies(),
                verify=True, 
                timeout=30
            )
    ...
    def download_file(self, server_relative_url: str) -> bytes:
        ...
        endpoint = f"/_api/web/GetFileByServerRelativeUrl('{server_relative_url}')/SValue"
        headers = self._get_headers()
        url = self._build_url(endpoint)
...
class MetadataManager:
    ...
    def __init__(self, cos_api, csv_file_name: str = "sharepoint_metadata.csv"):
        self.cos_api = cos_api
        self.csv_file_name = csv_file_name
        
    def get_metadata_by_filename(self, csv_path: str, file_name: str) -> Optional[Dict[str, Any]]:
    ...
    def write_metadata(self, metadata: DocumentMetadata, csv_path: str) -> None:
        ...
        try:
            ...
            raise IOError(f"Failed to write metadata to {csv_path}: {e}") from e
    ...
    def remove_metadata(self, csv_path: str, file_name: str) -> None:
        ...
        try:
            ...
            raise IOError(f"Failed to remove metadata from {csv_path}: {e}") from e
    
    @staticmethod
    def _create_empty_dataframe() -> pd.DataFrame:
        """Create empty DataFrame with proper columns."""
        columns = ["file_name", "url", "created_by", "last_modified", 
                  "nota_number", "language", "source"]
...
class DocumentProcessor:
    ...
    def __init__(self, api_client: SharePointAPIClient, cos_api, metadata_manager: MetadataManager):
    ...
    def delete_document(self, file_name: str, cos_folder_path: Path) -> None:
        ...
        file_path = (cos_folder_path / metadata["source"] / 
                    metadata["language"] / file_name)
    ...
    def _upload_document(self, doc: ProcessedDocument, cos_folder_path: Path) -> None:
        ...
        try:
            # Upload to COS
            destination_path = (cos_folder_path / doc.source / 
                              doc.language / file_name)
    ...
    def _log_unparseable_document(self, file_name: str, doc: ProcessedDocument) -> None:
        ...
        # This would integrate with your existing DocumentParser.write_unparsed_docs
        _, extension = os.path.splitext(file_name)
...
class SharePointClient:
    ...
    def __init__(self, config: 'SharePointConfig'):
        ...
        self.document_processor = DocumentProcessor(
            self.api_client, self.cos_api, self.metadata_manager
        )
    ...
    def run(self, args) -> None:
        ...
        cos_folder_path = Path(
            config_handler.get_config("document_parser")["document_object_cos_folder"]
        )
    ...
    def _process_documents_by_language(
        self, 
    ...
    def _initialize_azure_credentials(self) -> 'AzureCredentials':
        ...
        # This would use your existing AzureCredentials.from_env method
    ...
    def _create_cos_api():
        ...
        # This would use your existing create_cos_api function
    ...
    def _get_config_handler(project_name: str):
        ...
        # This would use your existing get_or_raise_config function
