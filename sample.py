...
workflow:
  rules:
    ...
    - if: $CI_COMMIT_TAG
      when: never
    # runs if merging to default branch
    - if: $CI_COMMIT_BRANCH == $CI_DEFAULT_BRANCH && $CI_COMMIT_MESSAGE =~ /^Merge branch/
    # don't run on default branch (except if it's merging)
...
test_image:
  ...
  needs: []
  rules:
    # don't run on default branch during merging
    - if: $CI_COMMIT_BRANCH == $CI_DEFAULT_BRANCH && $CI_COMMIT_MESSAGE =~ /^Merge branch/
      when: never
    # run on branches
    - if: $CI_COMMIT_BRANCH
...
pre_release_last_check:
  ...
  needs: []
  rules:
    # only run on default branch during merging
    - if: $CI_COMMIT_BRANCH == $CI_DEFAULT_BRANCH && $CI_COMMIT_MESSAGE =~ /^Merge branch/
