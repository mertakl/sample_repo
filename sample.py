...
def create_valid_eureka_docx(filename="test_document.docx", language="fr"):
    ...
    # Add main subtitle - use regular paragraph, not Subtitle style
    # Only one title should have depth=0 to avoid the error
    subtitle = doc.add_paragraph("Manuel d'utilisation complet")
    # Don't apply Subtitle style to avoid duplicate depth=0
    ...
    print("- UN SEUL titre principal (depth=0) avec style Subtitle")
    print("- Un sous-titre en texte normal (pas de depth=0)")
...
def create_dutch_version():
    ...
    # Add main title and subtitle - only ONE should have depth=0
    main_title = doc.add_paragraph("Hoofdgids voor Documentatie")
    main_title.style = "Subtitle"  # This will be depth=0
    
    # Subtitle as regular paragraph to avoid duplicate depth=0
    subtitle = doc.add_paragraph("Volledige gebruikershandleiding")
    # No special style applied
